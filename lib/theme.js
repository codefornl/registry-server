...
function runTheme(options, req, res) {
    ...
    Promise.resolve()
        .then(function () {
            if(options.resume) {
                return require(path.join(root, themeDirectory)); 
            } else {
                throw("No resume provided")
            }
        })
        ...
        .catch(function (error) {
            fs.readFileAsync(path.join(root, themeDirectory, 'package.json'), 'utf8').then(function (themePackageJson) {
                ...
                output += '<pre>';
                output += error.stack || error || "Unknown error";
                output += '</pre>';
                output += 'Theme Info:';
                output += '<pre>';
                ...
                output += '<pre>We just launched the official 0.0.0 version recently, please make sure you update before continuing.</pre>';
                if (options.resume) {
...
var getTheme = function (req, res) {
    console.log("Recieved theme request, try processing")
    console.log(req)
