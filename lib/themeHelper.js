...
var defaultResume = require('resume-schema').resumeJson;
 ...
 * @param {*} res 
 */
const controllerTheme = (req, res) => {
    console.info("Theme requested")
    let resume = _.cloneDeep(defaultResume);
    let options = {};

    if (req.body && req.body.resume) {
        console.info('Use posted resume');
        resume = req.body.resume;
    }

    if (req.body && req.body.themeOptions) {
        console.info('Use posted theme options');
        options = req.body.themeOptions;
    }
    getTheme(req.params.theme, (err, theme) => {
        if (err) {
            res.status(404).send({status: "error", message: "Failed to get theme", })
        } else {
            if (theme) {
                if(resume){
                    ...
                    res.send({status: "ok", message: "Theme is available and valid"})
                }
            } else {
                res.status(404).send({status: "error", message: "Failed to get theme", })
            }
